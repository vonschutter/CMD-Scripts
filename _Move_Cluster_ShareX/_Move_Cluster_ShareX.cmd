@echo %1 %2 %3 %4 %5 %6 %7 %8 %9 |find /i "-debug" &&(@echo on & set trace=echo & set debug=set ^& pause) || (@echo off & set trace=rem)
:::::::::::::::           Header          ::::::::::::
::
set _VER_%0=1.0.0
...
:INIT 
	...
	echo %1 %2 %3 %4 %5 %6 %7 %8 %9 |find /i "-make detailed template" >nul 2>&1 && (echo DFS Source URL, Destination Server ^(just the hostname^), Destination Drive ^(e.g. C-Z$^), Destination Cluster ^(Cluster name^), Destination Cluster Resource, Destination Target Root Path ^(not including DRIVE LETTER:^\^)>DetailedClusterMoveTemplate.csv&goto quit) 
	...
	::   a dagerous loop that could bring down a server. This variable will remain in the environment 
	...
	:: * A indicative title will also be set fore each termianl to avoiid confusion.
	::

	if DEFINED SPAWN (set _ME2X=// SPAWN // %_ME% &  goto SETINGS)

	echo %1 |find /i "-map-file" >nul 2>&1 && set _INFILE=%2
		if DEFINED _INFILE for /f "skip=1 tokens=1,2,3,4,5,6 delims=," %%a in (%_INFILE%) do if {%_DETECTOR%}=={off} (set /a count+=1 & start "SPAWN" /min cmd /I /k "set /a SPAWN=1 && %0 %%a %%b %%c %%d %%e %%f  -nodetect -dryrun"
			) else (
		set /a count+=1 & start "SPAWN" /min cmd /I /k "set /a SPAWN=1 && %0 %%a %%b")
	color 71 & echo. &echo %_ME% :: ( Launched %count% simultanious tasks...)& echo. &TITLE %_ME% :: ( Launched %count% simultanious tasks...)
...
:MAIN
	...
	:EXEC_NODETECT
		if DEFINED SPAWN (set _MEX= %_ME2X% :: MODE = %_STATUS%) else (set _MEX=%_ME% :: MODE = %_STATUS%)
			...
			Title %_MEX% ::
			@call :DisplayBanner %0
			@call :AssignComponentsFromStdIn %_#1% %_#2% %_#3% %_#4% %_#5% %_#6% %_#7% 
			@call :ValidatePhysicalPath "\\%_srcserver%\%_srcdrive%$\%_ServerRootPath%"
			@call :ValidatePhysicalPath %_PhysicalUNCTarget%
			TITLE  %_MEX% :: Please verify the following components:
				echo Press ^<CONTROL^> ^<C^> to exit, Press any key to see the task list& pause & cls 
...
:AssignComponentsFromStdIn
	...
	::
	if {%6}=={} (set _ERRMSG=Failed to execute %0!! Please check the syntax of the %_INFILE%& goto DispErr)
	call :GetPhysicalSourceFromDFS
	set _dstserver=%2
	set _dstdrive=%3
		echo %_dstdrive% find /i "$" && set %_dstdrive%=%_dstdrive:~0,1%
		echo %_dstdrive% find /i ":" && set %_dstdrive%=%_dstdrive:~0,1%
	set _CLUSTERNAME=%4
	set _CLUSTER-ShareNAME=%5
	set _ServerRootPath=%6
